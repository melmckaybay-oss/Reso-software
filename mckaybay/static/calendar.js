...
const Calendar = (() => {
  ...
  function dragOver(event, accomId) {
    ...
    if (row && accomId !== _dragFromAccomId) {
  ...
  async function dropOnRoom(event, toAccomId) {
    ...
    if (!_dragResId || toAccomId === _dragFromAccomId) return;
    ...
    if (!confirm(`Move ${res.first_name} ${res.last_name} from ${fromAccom?.name || "current room"} to ${toAccom?.name}?`)) {
      return;
    }

    try {
      // Update the room assignment — replace the matching room entry
  ...
  async function refresh() {
    ...
    container.innerHTML = `
      ...
      <p class="text-xs text-gray-400 mt-2">Click an empty cell to create a reservation • Click a coloured bar to edit • Drag a bar to a different room to move it</p>`;
  }
